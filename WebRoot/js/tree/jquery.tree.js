...
(function($) {
    ...
    $.fn.treeview = function(settings) {
        var dfop =
            {
                 ...
                 */
                cbiconpath: basepath+"/js/tree/images/",
        ...
        function dblnodeclick(e) {
        	 var path = $(this).attr("tpath");
             var et = e.target || e.srcElement;
        	 et = $(et).parent().parent();
        	 et=$(et).children(".bbit-tree-ec-icon") ;
             var item = getItem(path);
                 //+ if collapsed, expend it 
                 if ($(et).hasClass("bbit-tree-elbow-plus") || $(et).hasClass("bbit-tree-elbow-end-plus")) {
                     var ul = $(this).next(); //"bbit-tree-node-ct"
        ...
        function bindevent() {
            ...
            }).click(nodeclick).dblclick(dblnodeclick)
        ...
        function InitEvent(parent) {
            ...
            //var nodesa = $("li.bbit-tree-node>div>a", parent);
        }
        function reflash(itemId) {
            ...
            if (node.length > 0) {
                ...
                if (item) {
                    asnyloadc(item, true, function(data) {
                        ...
                        var ht = [];
                        var hhh=buildnode(item, ht, deep - 1, path, isend);
                        ht.shift();
                        ht.pop();
                        var li = node.parent();
                        li.html(ht.join(""));
            ...
            node.addClass("bbit-tree-selected");
        }
        function parentnode(itemId) {
            var nid = itemId.replace(/[^\w-]/gi, "_");
            var node = $("#" + id + "_" + nid);
            var path = node.attr("tpath");
        ...
        me[0].t = {
            ...
            parentnode: function(itemOrItemId) {
            	return parentnode(itemOrItemId);
            }
        };
        return me;
    };
    ...
    $.fn.parentnode = function(ItemOrItemId) {
        if (this[0].t) {
            return this[0].t.parentnode(ItemOrItemId);
        }
        return null;
    };
})(jQuery);