			...
			<div class="page-content">
				<div class="yz-frame-head">
					<div class="searchbox">
						<div class="row">
							<div class="col-xs-12">
								<e:yzbutton id="add_t_layoutSelectors" name="添加" cssClass="btn btn-success" />
						...
						<div class="yz-frame-body">
							<div class="table-box">
								<tgrid:jqGrid url="listLayoutSelectors.action" autowidth="true"
									dourl="delPicture.action" id="layoutSelectors" mtype="POST"
									...
									<tgrid:jqGridHead headvalue="'id','name','focusedImg','操作'">
									
									<tgrid:jqcol name="id" index="id" width="20" hidden="true"/>
									<tgrid:jqcol name="name" index="name" width="30"/>
									<tgrid:jqcol name="focusedImg" index="focusedImg" width="100"/>
...
//查看
function query(id){
	var rd =jQuery("#layoutSelectors").getRowData(id);
	jQuery().yzIframeDialog({id:"query_t_sysVer_s",iframeurl:"<%=basepath%>/admin/t_sysVer/showlayoutSelectorsInfo.action?id="+rd['id'],title:"详情"});
	$('#query_t_sysVer_s').modal('show');
}

//添加
$("#add_t_layoutSelectors").click(function(){
	jQuery().yzIframeDialog({id:"add_t_layoutSelectors_s",iframeurl:"<%=basepath%>/admin/t_sysVer/show_addLayoutSelectors.action",title:"添加"});
	$('#add_t_layoutSelectors_s').modal('show');
})

//修改
function update(id){
	var rd =jQuery("#layoutSelectors").getRowData(id);
	jQuery().yzIframeDialog({id:"edit_layoutSelectors_m",iframeurl:"<%=basepath%>/admin/t_sysVer/showEditnLayoutSelectorsWindow.action?id="+rd['id'],title:"修改"});
	$('#edit_layoutSelectors_m').modal('show');
}

//删除
function del(id){
	var rowData =jQuery("#layoutSelectors").getRowData(id);
	 modalDialogconfim("confim_sysuser_logoff","确定删除？",[{name:"是",fn:function(){
		 $.post('<%=basepath%>/admin/t_sysVer/del_layoutSelectors.action',{id:rowData['id'],isclose:2},function(data){
				if(data.status==0){
					modalDialogAlert("删除成功");
					refreshGrid('layoutSelectors');
