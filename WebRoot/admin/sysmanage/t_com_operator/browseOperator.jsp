	...
	<div class="page-content">
		<div class="yz-frame-head">
			<div class="searchbox">
				<div class="row">
					<div class="col-xs-12">
						<e:yzbutton id="add_operator" name="添加" cssClass="btn btn-success" power="106901"/>
						<div style="float: right; margin-right: 200px;">
							<input name="content" id="search_content"
								style="height: 32px; width: 250px" placeholder="可输入运营商名称进行查询" />
		...
		<div class="yz-frame-body">
			<div class="table-box">
				<tgrid:jqGrid url="listT_com_operator.action" autowidth="true"
					dourl="delOperator.action" id="t_com_operatorlist" mtype="POST"
					search="false" export="true" pageid="t_com_operatorpage"
					prmNames="{id:'sid'}" sortname="sid" del="false"
					ondblClickRow="preview" rowList="15,30,45">
					<tgrid:jqGridHead
						headvalue="'SID' , '所属上级运营商' , '运营商名称','昵称','联系方式','创建时间','操作'">
...
<e:yzact id="act">
	<e:yzactbutton onclick="xg" title="修改" power="106903"/>
	<e:yzactbutton onclick="del" title="删除" power="106902"/>
</e:yzact>
<script type="text/javascript">

//加载列表
$(document).ready(function(){
	var len=$("#t_com_operatorlist").getGridParam("width");
	//列的权限控制
	 if(<%=user.getAdmintype()%>==1 || <%=user.getAdmintype()%>==2){//运营商，运营商管理员
		 $("#t_com_operatorlist").setGridParam().hideCol("companyname");
	 }else{//0，null： 系统管理员，全部显示
		 
	 }
	
	 $("#t_com_operatorlist").setGridWidth(len).trigger("reloadGrid");
});

//添加
$("#add_operator").click(function(){
	jQuery().yzIframeDialog({id:"add_t_com_operator_m",iframeurl:"<%=basepath%>/admin/t_com_operator/preAddOperator.action",
											title : "添加运营商管理员"
										});
						$('#add_t_com_operator_m').modal('show');
					})
					

//修改
function xg(sid){
	var rd =jQuery("#t_com_operatorlist").getRowData(sid);
	jQuery().yzIframeDialog({id:"edit_t_com_operator_m",iframeurl:"<%=basepath%>/admin/t_com_operator/PreEditOperator.action?sid="+ rd['sid'],
							title : "修改信息"
						});
		$('#edit_t_com_operator_m').modal('show');
	}

	//删除
	function del(sid) {
		var rowData = jQuery("#t_com_operatorlist").getRowData(sid);
		if (sid != null)
			jQuery("#t_com_operatorlist").jqGrid('delGridRow', rowData['sid'], {
				top : 120,
				left : 250,
