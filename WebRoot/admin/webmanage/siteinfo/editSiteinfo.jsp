...
height:200px;
	...
	<form action="<%=basepath%>/admin/siteinfo/updateSiteinfo.action" enctype="multipart/form-data" method="post"> 
     <input type="hidden" name="id"  value="${siteinfo.id }">
      <div class="admin-s-body block">
       		<span><b style="color: red;">*</b>文案标题</span>
       		<div class="admin-s"><input type="text" name="title"   id="title"  value="${siteinfo.title }"/></div>
       	</div>
      	<div class="admin-s-body block">
...
<div class="admin-s">  
			        			...
			        			<c:forEach items="${fns:getCodeMap(pageContext.request,'sicategory')}" var="item">
			        			 <option value="${item.key}" ${siteinfo.sicategory==item.key?'selected="selected"':''}>${item.value}</option>
			        			 </c:forEach>  
			        			 </select>
       		</div>       		</div>
       	</div>
       		...
       		<div class="admin-s"><input type="text" name="silink"   id="silink"    value="${siteinfo.silink }"/>(当填写"链接"后点击文章标题将直接跳转至链接地址，不显示文章内容。链接格式请以http://开头)</div>
       	</div>
       	<div class="admin-s-body block">
       		<span> 文案图片</span>
       		<div class="admin-s"><input type="file" name="pic"   id="pic"/></div>(如不修改可不填写)
       	</div> 
       	<div class="admin-s-body block">
       		<span>状态</span>
       		<div class="admin-s">  
			        			...
			        			<c:forEach items="${fns:getCodeMap(pageContext.request,'isshow')}" var="item">
			        			  <option value="${item.key}" ${siteinfo.isshow==item.key?'selected="selected"':''}>${item.value}</option>
			        			 </c:forEach>  
			        			 </select>
       		</div>
       	</div>
       	<div class="admin-s-body block">
       		...
       		<div class="admin-s"><input type="text" name="px"  id="px"   value="${siteinfo.px }"  placeholder="请输入排序"  />(数字范围为0~255，数字越大越靠前
       		)</div>
       	</div>
   			 ...
   			 <script id="editor"  type="text/plain" style="width:100%;height:300px;"></script>
					...
					<c:if test="${meg!=null}">
						<e:msgdialog basepath="<%=basepath%>"
							callback="window.parent.closeDialogm('edit_siteinfo_m');window.parent.refreshGrid('siteinfo');">
							${meg}
						</e:msgdialog>
					</c:if>
