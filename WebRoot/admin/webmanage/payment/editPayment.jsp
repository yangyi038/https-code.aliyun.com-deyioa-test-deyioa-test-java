...
height:200px;
	...
	<form action="<%=basepath%>/admin/payment/updatePayment.action" enctype="multipart/form-data" method="post"> 
      		<input type="hidden" name="pay_id"  value="${payment.pay_id }">
      		<div class="admin-s-body block">
       		<span><b style="color: red;">*</b>支付方式名称</span>
       		<div class="admin-s"><input type="text" name="pay_name"   id="pay_name"  value="${payment.pay_name }"/></div>
       	</div>
       	<div class="admin-s-body block">
       		<span><b style="color: red;">*</b>支付方式描述</span>
       		<div class="admin-s"><input type="text" name="pay_desc"   id="pay_desc" value="${payment.pay_desc }"/></div>
       	</div>
       	<div class="admin-s-body block">
       		<span><b style="color: red;">*</b>支付方式代码</span>
       		<div class="admin-s"><input type="text" name="pay_code"   id="pay_code" value="${payment.pay_code }"/>( 汉字首字母或英文缩写)</div>
       	</div>
      	<div class="admin-s-body block">
       		<span>是否可用</span>
       		<div class="admin-s">
<div class="admin-s">  
			        			...
			        			<c:forEach items="${fns:getCodeMap(pageContext.request,'pay_enabled')}" var="item">
			        			     <option value="${item.key}" ${payment.enabled==item.key?'selected="selected"':''}>${item.value}</option>
			        			 </c:forEach>  
			        			 </select>
       		</div>       		
       		</div>
       	...
       	<div class="admin-s-body block">
       		...
       		<div class="admin-s">  
			        			...
			        			<c:forEach items="${fns:getCodeMap(pageContext.request,'is_cod')}" var="item">
			        			     <option value="${item.key}" ${payment.is_cod==item.key?'selected="selected"':''}>${item.value}</option>
			        			 </c:forEach>  
			        			 </select>
       		</div>
       	</div>
       	<div class="admin-s-body block">
       		...
       		<div class="admin-s">  
			        			...
			        			<c:forEach items="${fns:getCodeMap(pageContext.request,'is_online')}" var="item">
			        			    <option value="${item.key}" ${payment.is_online==item.key?'selected="selected"':''}>${item.value}</option>
			        			 </c:forEach>  
			        			 </select>
       		</div>
       	</div>
       	<div class="admin-s-body block">
       		...
       		<div class="admin-s"><input type="text" name="pay_order"  id="pay_order"  value="${payment.pay_order}"    placeholder="请输入显示顺序"  />(数字范围为0~255，数字越大越靠前
       		)</div>
       	</div>
					...
					<c:if test="${meg!=null}">
						<e:msgdialog basepath="<%=basepath%>"
							callback="window.parent.closeDialogm('edit_payment_m');window.parent.refreshGrid('payment');">
							${meg}
						</e:msgdialog>
					</c:if>
