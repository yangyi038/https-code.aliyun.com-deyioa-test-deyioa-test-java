...
<style>
					...
					<c:if test="${meg!=null}">
                 	...
                 	<div class="modaltable">
                 		...
                 		<div class="col-xs-12">
                 			<div class="searchCont">
                 				*活动名称:<input type="text" id="act_name"  name="act_name"  value="${activity.act_name }">
                 			</div>
                 			<div class="searchCont">
                 				*商品折扣:<input type="text" id="rebates" onblur="zk()" value="${activity.rebate }">
                 			</div>
                 			<div class="searchCont">
                 				*活动介绍:<input type="text" id="act_desc"  value="${activity.act_desc }">
                 			</div>
                 			<br> 
                 			<br> 
                 			<div class="searchCont">
                 				*活动开始时间:<input type="text" id="start_time"   name="start_time" onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:00:00'})" value="${activity.start_time }">
                 			</div>
                 			<div class="searchCont">
                 				*活动结束时间:<input type="text" id="end_time"  name="end_time"  onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:00:00'})" value="${activity.end_time }">
                 			</div> 
                 			</div>
                 			...
                 			<div class="scrolltable">
                 				...
                 				<c:forEach items="${goodsactivity}" var="item">
                 				<tr>
					<td   class="bh" id="goods_sn${item.goods_id}">${item.goods_sn }</td> 
					<td   class="name" id="goods_name${item.goods_id}">${item.goods_name }</td> 
					<td   class="price" id="shop_price${item.goods_id}">${item.shop_price }</td> 
					<td   class="kc" id="goods_number${item.goods_id}">${item.goods_number }</td> 
					<td   class="kc" > <input type="text" id="rebate${item.goods_id}" name="rebate"  value="${item.rebate }"  class="rebate number" style="width:60px"></td> 
				<td   class="kc"> 
				<select  name="isindex"  id="isindex${item.goods_id}"   > 
			        			<c:forEach items="${fns:getCodeMap(pageContext.request,'isindex')}" var="items">
			        			   <option value="${items.key}" ${item.isindex==items.key?'selected="selected"':''}>${items.value}</option>
			        			 </c:forEach>   
													</select>
													</td> 
					<td   class="zt"><input type="checkbox" name="actslist"  value="${item.goods_id}"  checked="checked"></td> 
			 		</tr> 
			        			 </c:forEach>   
                 				</table>
                 			</div>
           ...
           function addGoods(){
        	   ...
        	   $('input[name="goodslist"]:checked').each(function(){ 
        		 ...
        		 info+='<tr>'+
					...
					'<td   class="kc" > <input type="text" id="rebate'+goods_id+'" name="rebate"  class="rebate number" style="width:60px"></td>' +
           ...
           function addActGoods(){
    		 ...
    		 var isinde =   $('#isindex'+good_id).val();
        	   ...
        	   if(goods_id!=""){
        		   ...
        		   $.post("<%=basepath%>/admin/activity/updateActivity.action", {'act_id':act_id,'goods_id':goods_id,'goods_name':goods_name,'act_name':act_name,'act_desc':act_desc,'start_time':start_time,'end_time':end_time,'rebate':rebate,'rebates':rebates,'isindex':isindex}, function(data){
       				if (data.status==0) {
       				 
       					modalDialogconfim("edit_activity_m",data.info,[{name:"是",fn:function(){
       						closeDialog("edit_activity_m");
       					 window.parent.closeDialogm('edit_activity_m');
