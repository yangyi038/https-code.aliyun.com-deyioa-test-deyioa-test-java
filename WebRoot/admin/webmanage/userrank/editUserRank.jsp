...
height:200px;
	...
	<form action="<%=basepath%>/admin/userrank/updateUserRank.action" enctype="multipart/form-data" method="post"> 
      		<input type="hidden"  name="rank_id"  value="${userrank.rank_id }">
      		<div class="admin-s-body block">
       		<span><b style="color: red;">*</b>积分名称</span>
       		<div class="admin-s"><input type="text" name="rank_name"   id="rank_name" value="${userrank.rank_id }"/></div>
       	</div>
       	<div class="admin-s-body block">
       		<span><b style="color: red;">*</b>最低积分</span>
       		<div class="admin-s"><input type="text" name="min_points"   id="min_points" value="${userrank.min_points }"/></div>
       	</div>
       	<div class="admin-s-body block">
       		<span><b style="color: red;">*</b>最高积分 </span>
       		<div class="admin-s"><input type="text" name="max_points"   id="max_points" value="${userrank.max_points }"/> </div>
       	</div>
       	<div class="admin-s-body block">
       		<span><b style="color: red;">*</b>商品折扣 </span>
       		<div class="admin-s"><input type="text" name="discount"   id="discount"  value="${userrank.discount }"/> </div>
       	</div>
      	<div class="admin-s-body block">
...
<div class="admin-s">  
			        			...
			        			<c:forEach items="${fns:getCodeMap(pageContext.request,'show_price')}" var="item">
			        			     <option value="${item.key}" ${userrank.show_price==item.key?'selected="selected"':''}>${item.value}</option>
			        			 </c:forEach>  
			        			 </select>
       		</div>       		
       		</div>
       	...
       	<div class="admin-s-body block">
       		...
       		<div class="admin-s">  
			        			...
			        			<c:forEach items="${fns:getCodeMap(pageContext.request,'special_rank')}" var="item">
			        			     <option value="${item.key}" ${userrank.special_rank==item.key?'selected="selected"':''}>${item.value}</option>
			        			 </c:forEach>  
			        			 </select>
       		</div>
       	</div>   
       	<div class="admin-s-body block">
       		...
       		<div class="admin-s">  
			        			...
			        			<c:forEach items="${fns:getCodeMap(pageContext.request,'usertype')}" var="item">
			        			     <option value="${item.key}" ${userrank.usertype==item.key?'selected="selected"':''}>${item.value}</option>
			        			 </c:forEach>  
			        			 </select>
       		</div>
       	</div>   
					...
					<c:if test="${meg!=null}">
						<e:msgdialog basepath="<%=basepath%>"
							callback="window.parent.closeDialogm('edit_userrank_m');window.parent.refreshGrid('userrank');">
							${meg}
						</e:msgdialog>
					</c:if>
